...
export function detectMime(bytes: Uint8Array): string | null {
  // Check PDF signature: anywhere in the first 1024 bytes per ISO 32000-1 specification
  const checkLimit = Math.min(bytes.length, 1024);
  for (let i = 0; i <= checkLimit - 5; i++) {
