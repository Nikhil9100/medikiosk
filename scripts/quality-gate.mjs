...
const ui=["app/patient/page.tsx","app/doctor/page.tsx","app/hospital/page.tsx"].map(x=>read(path.join(root,x))).join("\n");ok(!/Ashoka|Government of India|Ministry of Health/i.test(ui),"UI does not imply Government of India ownership");ok(ui.includes("independent SIH solution")||ui.includes("Independent SIH"),"institutional UI contains independence disclosure");ok(fs.existsSync(path.join(root,"CHECKPUSH.md")),"CHECKPUSH.md exists at package root");
