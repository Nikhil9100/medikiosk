...
export async function POST(request:Request){
  ...
  try{const s=await withOwnerTx(ownerId,c=>createCase(c,{ownerId,idempotencyKey:key,language:"en",resumeTokenHash}));const res=NextResponse.json({workflow:caseToWorkflow(s),resumeEnabled:Boolean(resumeTokenHash)},{status:201,headers:{"Cache-Control":"no-store, private"}});setCookie(res,s.id);return res;}catch(e){if(e instanceof Error&&e.message==="IDEMPOTENCY_RESUME_MISMATCH")return NextResponse.json({error:"Session retry credentials do not match",code:"IDEMPOTENCY_CONFLICT"},{status:409});return NextResponse.json({error:"Unable to create secure session"},{status:500});}
}

export async function GET(){
  if(!databaseConfigured())return NextResponse.json({error:"Database not configured",code:"DB_NOT_CONFIGURED"},{status:503});
  const id=await sessionIdFromCookie();if(!id)return NextResponse.json({error:"No session"},{status:404});
  try{const s=await withKioskTx(id,async c=>{const current=await getCase(c,id);if(current&&current.status==="ACTIVE"&&current.expiresAt.getTime()>Date.now())await touchPatientSession(c,id);return current;});if(!s)return NextResponse.json({error:"No session"},{status:404});if(s.status!=="ACTIVE"||s.expiresAt.getTime()<=Date.now())return NextResponse.json({error:"Session ended",code:"SESSION_EXPIRED"},{status:410});return NextResponse.json({workflow:caseToWorkflow(s)},{headers:{"Cache-Control":"no-store, private"}});}catch{return NextResponse.json({error:"Session unavailable"},{status:500});}
}
...
export async function PATCH(request:Request){
  ...
  try{const s=await withKioskTx(id,c=>patchCase(c,id,parsed.data));if(!s)return NextResponse.json({error:"Session not writable"},{status:410});return NextResponse.json({workflow:caseToWorkflow(s)},{headers:{"Cache-Control":"no-store, private"}});}catch(e){if((e as Error).message==="CASE_READ_ONLY")return NextResponse.json({error:"Submitted cases are read-only"},{status:409});return NextResponse.json({error:"Unable to save"},{status:500});}
}
