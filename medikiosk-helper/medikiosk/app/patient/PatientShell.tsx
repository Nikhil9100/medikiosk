 ...
 return <Ctx.Provider value={value}>
   ...
   <div className="patient-app reference-patient-app">
    <header className="patient-topbar reference-patient-header">
      <div className="patient-brand"><span className="brand-mark" aria-hidden="true"><PatientIcon name="cross" size={22}/></span><div><strong>MediKiosk</strong><span>{translate(workflow.language,"patientVisit")} · {translate(workflow.language,"tagline")}</span></div></div>
      <div className="patient-top-actions">
        <button type="button" className="emergency-trigger-btn" onClick={()=>setEmergencyModalOpen(true)} aria-label="Emergency Help"><PatientIcon name="alert" size={18}/><span>{translate(workflow.language,"emergencyTriggerBtn") || "Get Help"}</span></button>
        <button
          type="button"
